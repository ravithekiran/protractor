...
var HTMLReport = function() {
    ...
    var generateSummaries = function(reportXml, report) {
        ...
        for (var i=0; i<totalSuites; i++) {
            ...
            for (var j=0; j<totalCasesPerSuite; j++) {
                ...
                if (testCasesResults[j] != 'Failed' && report.screenshotsOnlyOnFailure) {
                    ...
                    if (report.modifiedSuiteName) {
                        screenshotsNamesOnFailure.push(testCasesNames[j] + '.png');
            ...
            //store suite data
            suitesData.push({'keyword': 'TestCase', 'name': suite.name, 'testcases': testCasesNames, 'testcasesresults':testCasesResults, 'testcasestimes': testCasesTimes, 'testcasesmessages': testCasesMessages, 'screenshotsNames':screenshotsNamesOnFailure, 'tests': suite.tests, 'failed': suite.failed, 'errors': suite.errors, 'skipped': suite.skipped, 'passed': suite.passed});
    ...
    this.from = function(reportXml, testConfig) {
        ...
        report.platform=testConfig.platform|| 'unknownPlatform'
        ...
        //write to html file
        var testOutputPath = getPath('/' + 'Test-report.html', testConfig['outputPath'] || './');
